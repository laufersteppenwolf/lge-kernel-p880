...
void __init tegra_init_cpu_edp_limits(unsigned int regulator_mA)
{
	...
	for (j = 0; j < edp_limits_size; j++) {
		...
		e[j].freq_limits[1] = (unsigned int)(t[i+j].freq_limits[1]+50) * 10000;
		e[j].freq_limits[2] = (unsigned int)(t[i+j].freq_limits[2]+50) * 10000;
		e[j].freq_limits[3] = (unsigned int)(t[i+j].freq_limits[3]+50) * 10000;
        }
